...
$q1=mysql_query("select * from province order by province_name asc");
//echo mysql_error();
while($nt1=mysql_fetch_array($q1)){
	
	echo "addOption(document.drop_list.group_car, '$nt1[province_id]', '$nt1[province_name]');";
}// end of while
?>
...
addOption(document.drop_list.sub_car, "", "เลือกอำเภอ", "");
...
$q2=mysql_query("select distinct(province_id) from amphur order by amphur_name asc");
// In the above query you can collect cat_id from category table also. 
while($nt2=mysql_fetch_array($q2)){
//echo "$nt2[cat_id]";
echo "if(document.drop_list.group_car.value == '$nt2[province_id]'){";
$q3=mysql_query("select amphur_name,amphur_id from amphur where province_id='$nt2[province_id]'");
while($nt3=mysql_fetch_array($q3)){
echo "addOption(document.drop_list.sub_car,'$nt3[amphur_id]', '$nt3[amphur_name]');";
