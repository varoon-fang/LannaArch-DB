...
<?php $this->load->view('backoffice/components/page_subhead');?>
      ...
      <?php
      		$i=0;
			$sql="select * from banner_top order by banner_top_num asc";
				$rs=$this->db->query($sql)->result();
    		
    		 foreach($rs as $row){  
    		 	$id = $row->banner_top_id;
    		 	$name = $row->banner_top_name;
    		  $i++;
      ?>
	    <li class="span3" <? if($i==1){ echo "style='margin-left:20px'";}else{};?> >
		    ...
		    <img src="<?= site_url();?>/images/banner/resize/<?=$name;?>" alt="<?=$name;?>">
			    <div class="caption">
					<p align="center"><a href="<?= site_url();?>/backoffice/banner/delete_banner2/<?= $id;?>" onclick="return confirm('Delete?')" class="btn btn-danger btn-block">Delete</a></p>
			    </div>
		    </div>
	    </li>
	    <? }?>
        ...
        <?= form_open_multipart('backoffice/banner/add_banner2', array('id' =>'contact-form', 'name' =>'myform'));?>
	    	...
	    	<div class="well">
	                	...
	                	<p><span class="label label-info">คำแนะนำ</span> ขนาดรูปไม่ต่ำกว่า 698 x 148 px  สามารถอัพได้หลายรูปด้วยการกด Shift และกดรูป </p>
