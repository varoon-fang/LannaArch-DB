...
<?php $this->load->view('admin/components/page_subhead');?>
         ...
         <h3><a href="<?= site_url("admin/activity/view/$rs[activity_group]");?>"><i class="icon-arrow-left back-link"></i></a> แก้ไขข้อมูลกิจกรรม</h3>
         <br />
         <? echo $this->session->flashdata('feedback_edit');?>

        <?= form_open("admin/activity/edit/".$this->uri->segment(4), array('id' =>'contact-form', 'name' =>'myform'));?>
		  ...
		  <div class="well">
	    	  ...
	    	  <div class="controls">
	    	    <select name="group" required="reauired">
	    	    	<?php
	    	    		...
	    	    		{
	    	    	?>
	    	    	<option value="<?= $row['activity_group_id'];?>" <? if($row['activity_group_id']==$rs['activity_group']){ echo "selected";}else{}?>><?= $row['activity_group_name'];?></option>
	    	    	<? }?>
	    	    </select>
	    	  </div>
	    	</div>
	    	...
	    	<div class="control-group">
	    	  ...
	    	  <div class="controls">
	    	    <input type="text" placeholder="กรอกข้อความ…" name="title_th" value="<?= $rs['activity_title'];?>" class="span6" id="title_th">
	    	  </div>
	    	</div>
		    ...
		    <div class="control-group">
			  ...
			  <div class="controls">
			    <textarea id="detail_th" name="detail_th" rows="7"  class="span6" placeholder="กรอกข้อความ…"><?= $rs['activity_detail'];?></textarea>
			  </div>
			</div>
              ...
              <p><span class="badge badge-inverse">2.</span> <strong>( ข้อมูลรูปภาพ )</strong></p>
                 <br />
							...
							<?php
								...
								$i++;
									$name_img = $fett['activity_album_name'];
									$id_img = $fett['activity_album_id'];
							?>
							<li class="span2" >
							    <div class="thumbnail">
							    <img src="<?= site_url("images/activity/resize/$name_img");?>" alt="<?=$name_img;?>">
								    <div class="caption">
										<p align="center"><a href="<?= site_url("admin/activity/delete_image/$id_img");?>" onclick="return confirm('Delete?')" class="btn btn-danger btn-block">Delete</a></p>
								    </div>
							    </div>
						    </li>
  ...
  <!-- Upload image -->
    ...
    <?= form_open_multipart('admin/activity/upload_pic', array('id' =>'contact-form', 'name' =>'myform'));?>

	   	<input  type="hidden" value="<?= $rs['activity_id'];?>" name="id_activity" >
