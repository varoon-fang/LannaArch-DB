...
<?php $this->load->view('admin/components/page_subhead');?>
         ...
         <h3><a href="<?= site_url("admin/models");?>"><i class="icon-arrow-left back-link"></i></a> แก้ไขข้อมูลภาพหุ่นจำลอง</h3>
         <br />
          <? echo $this->session->flashdata('feedback_edit');?>
        <?= form_open("admin/models/edit/".$this->uri->segment(4), array('id' =>'contact-form', 'name' =>'myform'));?>
    ...
    <!-- // inputnormal -->
		  <div class="well">
	    	...
	    	<div class="control-group">
	    	  ...
	    	  <div class="controls">
	    	    <input type="text" placeholder="กรอกข้อความ…" name="title_th" value="<?= $rs['models_title'];?>" class="span6" id="title_th">
	    	  </div>
	    	</div>
		    ...
		    <div class="control-group">
			  ...
			  <div class="controls">
			    <textarea id="detail_th" name="detail_th" rows="7"  class="span6" placeholder="กรอกข้อความ…"><?= $rs['models_detail'];?></textarea>
			  </div>
			</div>
              ...
              <p><span class="badge badge-inverse">2.</span> <strong>( ข้อมูลรูปภาพ )</strong></p>
                 <br />
                 	<? echo $this->session->flashdata('feedback');?>
							...
							<?php
								$i=0;
								foreach($rs_models_img->result_array() as $fett){
								$i++;
									$name_img = $fett['models_album_name'];
									$id_img = $fett['models_album_id'];
							?>
							<li class="span2" >
							    <div class="thumbnail">
							    <img src="<?= site_url("images/models/resize/$name_img");?>" alt="<?=$name_img;?>">
								    <div class="caption">
										<p align="center"><a href="<?= site_url("admin/models/delete_image/$id_img");?>" onclick="return confirm('Delete?')" class="btn btn-danger btn-block">Delete</a></p>
								    </div>
							    </div>
						    </li>
	            			<? }?>
  ...
  <!-- Upload image -->
    ...
    <?= form_open_multipart('admin/models/edit_pic', array('id' =>'contact-form', 'name' =>'myform'));?>

	   	<input  type="hidden" value="<?= $rs['models_id'];?>" name="id_models" >
