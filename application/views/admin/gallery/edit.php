...
<?php $this->load->view('admin/components/page_subhead');?>
         ...
         <h3><a href="<?= site_url("admin/gallery/view/$rs[gallery_group]");?>"><i class="icon-arrow-left back-link"></i></a> แก้ไขข้อมูลห้องสมุดภาพ</h3>
         <br />
         <? echo $this->session->flashdata('feedback_edit');?>

        <?= form_open("admin/gallery/edit/".$this->uri->segment(4), array('id' =>'contact-form', 'name' =>'myform'));?>
		  ...
		  <div class="well">
	    	  ...
	    	  <div class="controls">
	    	    <select name="group" required="reauired">
	    	    	<?php
	    	    		...
	    	    		{
	    	    	?>
	    	    	<option value="<?= $row['gallery_group_id'];?>" <? if($row['gallery_group_id']==$rs['gallery_group']){ echo "selected";}else{}?>><?= $row['gallery_group_name'];?></option>
	    	    	<? }?>
	    	    </select>
	    	  </div>
	    	</div>
	    	...
	    	<div class="control-group">
	    	  ...
	    	  <div class="controls">
	    	    <input type="text" placeholder="กรอกข้อความ…" name="title_th" value="<?= $rs['gallery_title'];?>" class="span6" id="title_th">
	    	  </div>
	    	</div>
		    ...
		    <div class="control-group">
			  ...
			  <div class="controls">
			    <textarea id="detail_th" name="detail_th" rows="4"  class="span6" placeholder="กรอกข้อความ…"><?= $rs['gallery_from'];?></textarea>
			  </div>
			</div>
              ...
              <p><span class="badge badge-inverse">2.</span> <strong>( ข้อมูลรูปภาพ )</strong></p>
                 <br />
							...
							<?php
								...
								$i++;
									$name_img = $fett['gallery_album_name'];
									$id_img = $fett['gallery_album_id'];
							?>
							<li class="span2" >
							    <div class="thumbnail">
							    <img src="<?= site_url("images/gallery/resize/$name_img");?>" alt="<?=$name_img;?>">
								    <div class="caption">
										<p align="center"><a href="<?= site_url("admin/gallery/delete_image/$id_img");?>" onclick="return confirm('Delete?')" class="btn btn-danger btn-block">Delete</a></p>
								    </div>
							    </div>
						    </li>
  ...
  <!-- Upload image -->
    ...
    <?= form_open_multipart('admin/gallery/upload_pic', array('id' =>'contact-form', 'name' =>'myform'));?>

	   	<input  type="hidden" value="<?= $rs['gallery_id'];?>" name="id_gallery" >
