...
function insert_cate()
{
	...
	{
            $data=array(
        	'activity_group_name' => $this->input->post('title_th'),

        );
        $query = $this->db->insert('activity_group', $data);
...
function edit_cate($id)
{
	...
	if($this->input->post('send')!=NULL)
	{
         ...
         $this->db->where('activity_group_id', $id);
        $query = $this->db->update('activity_group', $data);
...
{
	// delete activity
	$query_act = $this->db->get_where('activity', array('activity_group' => $id));
		foreach ($query_act->result_array() as $row)
		{
			$act_id = $row['activity_id'];
		}
	$query = $this->db->get_where('activity_album', array('activity_id' => $act_id));

			foreach ($query->result() as $row)
			{
			     $img = $row->activity_album_name;
			     $del = unlink("images/activity/thumbs/$img");
			     $del2 = unlink("images/activity/resize/$img");

			}

    $this->db->where('activity_id', $act_id);
    $this->db->delete('activity_album');

	$this->db->where('activity_group', $id);
	$query = $this->db->delete('activity');

	//delete group
	$this->db->where('activity_group_id', $id);
	$query = $this->db->delete('activity_group');
		if($query)
        {
        	$this->session->set_flashdata('feedback', ' <div class="alert alert-error" id="alert-message">
			    <button type="button" class="close" data-dismiss="alert">&times;</button>
...
// ------------------------ Activity Mode ------------------------
	function insert(){
        ...
        //add data
	    	$data = array(
	    	   'activity_group'		=> $this->input->post('group'),
	    	   'activity_title' 	=> ($this->input->post('title_th')),
			   'activity_detail' 	=> ($this->input->post('detail_th')),
        ...
        foreach($_FILES as $field_name => $file)
        {
            ...
            else
            {
                ...
                else
                {
                  ...
                  // select activity
                  $sql="select * from activity order by activity_id desc limit 1";
    		...
    		// add images
                    $data_img=array(
                	'activity_id' => "$idd",
                	'activity_album_name' => $upload_data['file_name'],
                	'activity_album_num' => $j++,
                );
                $query = $this->db->insert('activity_album', $data_img);
	...
	function edit_data($id){
		if($this->input->post('del_img')!=NULL){
			foreach( $this->input->post('img_id') as $value ):
			    ...
			    //echo $value;
			    $query = $this->db->get_where('activity', array('activity_id' => $value));

				foreach ($query->result() as $row)
				{
				     $img = $row->activity_img;
				     $del = unlink("images/activity/thumbs/$img");
				     $del2 = unlink("images/activity/resize/$img");
			    ...
			    $this->db->where('activity_id', $value);
	...
	function update_pic(){
        ...
        $upload_conf = array(
            'upload_path'   => realpath('images/activity/'),
            'allowed_types' => 'gif|jpg|png',
            'quality' 		=> '80',
            'max_size'      => '0',
            'encrypt_name'	=> TRUE,
            ...
            else
            {
               ...
               $resize = array(
                ...
                else
                {
                    ...
                    unlink($upload_data['full_path']);
             //count img
             	$sql = $this->db->get_where('activity_album', array('activity_id' => $id));
             		$count= $sql->num_rows();
			 		$j=$count+1;
    		// add images
    			$idd=$this->input->post('id_activity');
                    $data_img=array(
                	'activity_album_num' => $j++,
                	'activity_album_name' => $upload_data['file_name'],
                	'activity_id' =>$id
...
function delete_img($id)
    {
    	...
    	$query = $this->db->get_where('activity_album', array('activity_album_id' => $id));
        ...
        $this->db->where('activity_album_id', $id);
        $query = $this->db->delete('activity_album');
...
function delete()
    {
    	$id = $this->uri->segment(4);
    	$query = $this->db->get_where('activity_album', array('activity_id' => $id));
